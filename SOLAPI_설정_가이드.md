...
- **API 키 형식**: `NCSFMJ9TRACUIW` (16자리) 또는 `NCSFMJ9TRVXACUIW` (32자리 영문자+숫자)
- **API Secret 형식**: `HQGV2DZ4CC0UV7LZ2TWLW0O3VSHX53VO` (32자리 영문자+숫자)
- **참고**: SOLAPI는 16자리와 32자리 두 가지 형식의 API 키를 지원합니다
...
#### 1. 잘못된 API 키 형식
- **증상**: HTTP 400 오류
- **원인**: API 키가 16자리 또는 32자리가 아니거나 특수문자 포함
- **해결**: SOLAPI 대시보드에서 새로운 API 키 재생성
- **참고**: SOLAPI는 16자리와 32자리 두 가지 형식의 API 키를 지원합니다

#### 2. 인증 방식 오류
- **증상**: HTTP 400 또는 401 오류
- **원인**: API Key와 Secret을 요청 본문에 포함
- **해결**: 자동으로 요청 본문 방식으로 처리됨
