...
- **다중 사용자 이력 관리**
- **벡터 검색 기반 지식 베이스**
- **✅ 데이터 영구 저장**: Streamlit Cloud에서도 서버 재부팅 시 데이터 유지**
- **✅ 자동 백업 시스템**: 데이터 손실 방지를 위한 백업 메커니즘**
- **✅ 클라우드 최적화**: Streamlit Cloud 환경에 최적화된 저장소 시스템**
...
### 4. 데이터 관리 (새로운 기능)
- **📦 전체 데이터 백업**: 사용 가이드 탭에서 백업 생성
- **🔄 데이터 상태 확인**: 현재 저장된 데이터 개수 및 경로 확인
- **🧹 임시 파일 정리**: 백업 파일 등 임시 파일 정리
- **✅ 자동 복구**: 서버 재부팅 시 모든 데이터 자동 복구
...
- **데이터 처리**: Pandas, NumPy
- **API 통신**: Requests
- **데이터 저장**: JSON 기반 영구 저장소 + 자동 백업 시스템
- **클라우드 지원**: Streamlit Cloud 최적화
